langs.zhTW = {
  ...
  '大陆首选 亚洲区最佳 Network': '大陸首選 亞洲區最佳 Network',
  '打开 Telegram 搜索': '打開 Telegram 搜尋',
  '立即尝试多协议 Network 服务': '立即嘗試多協議 Network 服務',
  '银行户口、电邮密码、个人资料': '銀行戶口、電郵密碼、個人資料',
  '什么是 Network ？它有何帮助？': '什麼是 Network ?它有何幫助?',
  '借道无关 IP 网上冲浪更匿名': '借道無關 IP 網上衝浪更匿名',
  '多组服务器集群覆盖多个国家地区': '多組伺服器集群覆蓋多個國家地區',
  '最先进开源协议和我们的自研秘技': '最先進開源協議和我們的自研秘技',
  '你们的 Network 能在哪些设备上运作？': '你們的 Network 能在哪些設備上運作？',
  ...
  '为什么你们的私有 Network 协议能做到稳定高速？': '為什麼你們的私有 Network 協議能做到穩定高速?',
  ...
  '最先进流量隐藏技术，将Network穿墙流量化身网页浏览，免于防火墙干扰。': '最先進流量隱藏技術,將Network穿牆流量化身網頁瀏覽,免於防火牆干擾。',
  '私密浏览？浏览器无痕模式不够。我们的Network为您隐藏IP以掩藏踪迹。': '私密瀏覽?瀏覽器無痕模式不夠。我們的Network為您隱藏IP以掩藏蹤跡。',
  ...
  '影-梭R 是前者的主要分支，修补了安全漏洞，扩展了加密选项，增加了流量混淆能力。':
    '影-梭R 是前者的主要分支,修補了安全漏洞,擴展了加密選項,增加了流量混淆能力。',
  ...
  '在防火墙检查时，混淆后的 Network 流量看似普通网上冲浪或电影串流。因此，审查者无法确定哪些流量需要阻止，哪些流量可以放行。':
    '在防火牆檢查時,混淆後的 Network 流量看似普通網上衝浪或電影串流。因此,審查者無法確定哪些流量需要阻止,哪些流量可以放行。',
  '影-梭 是一种无连接无状态的代理协议。它承载 TCP UDP 数据，可轻松穿透世界上最强国家防火墙，中国的 GFW 。':
    '影-梭 是一種無連接無狀態的代理協議。它承載 TCP UDP數據,可輕鬆穿透世界上最強國家防火牆,中國的 GFW 。',
  '我们自研的私有协议，可以模拟多种网页浏览场景。使得防火墙无法可靠区分 Network 数据浏览。如强行阻拦亦会影响正常使用网络，因此连接无法有效拦截。':
    '我們自研的私有協議,可以模擬多種網頁瀏覽場景。使得防火牆無法可靠區分 Network 數據瀏覽。如強行阻攔亦會影響正常使用網絡,因此連接無法有效攔截。',
  'Network 提供了一系列好处：通过加密，通讯数据变得难以破解；藉由隧道，线上活动的细节内容被掩藏；借助转发，网路流量看似来自无关的 IP 地址和地理位置。':
    'Network 提供了一系列好處:通過加密,通訊數據變得難以破解;藉由隧道,線上活動的細節內容被掩藏;借助轉發,網路流量看似來自無關的 IP 地址和地理位置。',
  ...
  '市面上几乎所有的电子设备，我们都能支持。鉴于我们支援多种开源和自研的 Network 协议，我们相信您可以在几乎任何市售设备上轻松连接到我们的 Network 网络。':
    '市面上幾乎所有的電子設備,我們都能支持。鑒於我們支援多種開源和自研的 Network 協議,我們相信您可以在幾乎任何市售設備上輕鬆連接到我們的 Network 網絡。',
  'Network 是虚拟专用网络（Virtual Private Network）的缩写。它通过在公共网络上建立专用隧道并加密其中传输的数据来保护两个或多个设备之间的通信。':
    'Network 是虛擬專用網絡(Virtual Private Network)的縮寫。它通過在公共網絡上建立專用隧道並加密其中傳輸的數據來保護兩個或多個設備之間的通信。',
  '影-梭（简称 影-梭，又称影梭）是一种加密的无状态代理协议。它在传输层工作，中继 TCP 和 UDP 流量。它可以轻松穿透最强大的国家级防火墙（中国的 GFW ）。':
    '影-梭(簡稱 影-梭,又稱影梭)是一種加密的無狀態代理協議。它在傳輸層工作,中繼 TCP 和 UDP 流量。它可以輕鬆穿透最強大的國家級防火牆(中國的 GFW )。',
  ...
  '影-梭R（简称 影-梭R）是 影-梭 的主要分支。在 影-梭 的基础上，影-梭R 修补了安全漏洞，扩展了密码套件选项，增加了流量混淆，并提高了运营商级别的 QoS 优先级。':
    '影-梭R(簡稱 影-梭R)是 影-梭 的主要分支。在 影-梭 的基礎上,影-梭R 修補了安全漏洞,擴展了密碼套件選項,增加了流量混淆,並提高了運營商級別的 QoS 優先級。',
  '交通混淆和质押自由是有效规避互联网封锁的基石。混淆将 Network 流量伪装成常规的 Web 浏览，审查者无从分辨；质押自由意味着，由于误封的成本过高，审查机构不会阻止无法明确分辨的流量。':
    '交通混淆和質押自由是有效規避互聯網封鎖的基石。混淆將 Network 流量偽裝成常規的 Web 瀏覽,審查者無從分辨;質押自由意味著,由於誤封的成本過高,審查機構不會阻止無法明確分辨的流量。',
  'TCP2 是一个流量转发软件平台。它支持多种代理协议，并允许这些协议与多种传输层协议配合使用。而 TCP 是这个平台上原创的代理协议。与 TLS 传输方式配合使用的时候，他可以完美模拟 HTTPS 上网浏览。':
    'TCP2 是一個流量轉發軟件平台。它支持多種代理協議,並允許這些協議與多種傳輸層協議配合使用。而 TCP 是這個平台上原創的代理協議。與 TLS 傳輸方式配合使用的時候,他可以完美模擬 HTTPS 上網瀏覽。',
  ...
  '用户协议完整内容': `
    <h6>特別提示</h6>
        <p>
          ...
          {appName} 與用戶之間關於」影-梭R「服務（以下簡稱「服務」）
